...
def calculate_stack_lambda(base_value, first_item, stack_effect, stack_type, limit=None):
    ...
    if stack_type == 'linear':
        def effect(i):
            (np.array(i) < limit[1]) * (
                    base_value + (np.array(i) > 0) * first_item + (np.array(i) > 1) * (stack_effect * (i - 1))) + (
                    np.array(i) >= limit[1]) * limit[0]

    elif stack_type == 'hyperbolic':
        def effect(n):
            1 - 1 / (stack_effect * n + 1)
        ...
        def uncommon_rarity(n):
            n * 20 / (80 + 20 * n + n ** 2) * 100

        def legendary_rarity(n):
            n ** 2 / (80 + 20 * n + n ** 2) * 100
        ...
        def effect(n):
            base_value / (1 - (np.array(n) > 0) * (-1) + (np.array(n) > 0) * first_item + (np.array(n) > 1) * (
                        stack_effect * (n - 1)))

    elif stack_type == 'corpsebloom':
        def effect(n): first_item * stack_effect ** (n - 1)
...
def item_stacking():
    ...
    item, rarity, effect = select_item_gui()
